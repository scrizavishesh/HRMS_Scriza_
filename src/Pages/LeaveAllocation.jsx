...
import { getLeasveALLoc } from '../utils/Constants';
...
const LeaveAllocation = () => {
    ...
    const GetLeavesAll = async () => {
        const response = await getLeasveALLoc();
        try {
            if (response?.status === 200) {
                toast.success(response?.data?.message);
                setLeavesAll(response?.data?.data)
            } else {
                ...
                <div className="row">
                    <div className="col-12" style={{ background: "#fff", overflowX: "auto" }}>
                        <table className="table text text-center">
                            ...
                            <tbody>
                                {LeavesAll?.length !== 0 ? (
                                    LeavesAll?.map((employ, index) => {
                                        return (
                                            <tr>
                                                <th><input class="form-check-input mt-0" type="checkbox" value="" aria-label="Checkbox for following text input" /></th>
                                                <td>{index + 1}</td>
                                                <td>{employ?.type}</td>
                                                <td>{employ?.annual_allocation}</td>
                                                <td>{employ?.status}</td>
                                                <td>
