...
import { departmentGet } from '../utils/Constants';
...
const AllDepartments = () => {
    ...
    const [allDepartemnt, setallDepartemnt] = useState("");

    const tooltips = [
        { id: 'eyeOne', template: 'View', icon: 'feather-eye' },
        { id: 'editOne', template: 'Edit', icon: 'feather-edit' },
        { id: 'trashOne', template: 'Delete', icon: 'feather-trash-2' },
    ];

    tooltips.forEach(({ id, template, icon }) => {
        tippy(`#${id}`, {
            // content: template,
            arrow: true,
            theme: 'light', 
            ...
            content: `<div class="tooltip-content"><span>${template}</span></div>`,
            trigger: 'mouseenter',
            placement: 'top',
            arrowType: 'round',
    ...
    const getDepartment = async (e) => {
        ...
        console.log(response, "All Departemnts");
        try {
            if (response?.status === 200) {
                toast.success(response?.data?.message);
                setallDepartemnt(response?.data?.data)
            } else {
                    ...
                    <div className="col-md-5">
                        <nav className="p-2" style={customStyle} aria-label="breadcrumb">
                            <ol className="breadcrumb">
                                ...
                                <li className="breadcrumb-item active" aria-current="page">All Departments</li>
                            </ol>
                        </nav>
                    </div>
                                ...
                                <div class="card-body">
                                    <div class="table-responsive table-card">
                                        <table class="table text-nowrap mb-0 table-centered table-hover">
                                            ...
                                            <tbody>
                                                {allDepartemnt?.length !== 0 ? (
                                                    allDepartemnt?.map((department) => {
                                                        return (
                                                            <tr role="row" class="odd">
                                                                <td class="pe-0 dtr-control" tabindex="0">
                                                                    <div class="form-check">
                                                                ...
                                                                {department.status === 0 ? <td className='text text-center'><span class="badge badge-success-soft">Active</span></td> : <td className='text text-center'><span class="badge badge-danger-soft">Inactive</span></td>}
                                                                <td>
                                                                    <a href="#!" class="btn btn-ghost btn-icon btn-sm rounded-circle texttooltip" data-template="eyeOne" id="eyeOne" >
                                                                    ...
                                                                    <a href="#!" class="btn btn-ghost btn-icon btn-sm rounded-circle texttooltip" data-template="editOne" id="editOne">
                                                                    ...
                                                                    <a href="#!" class="btn btn-ghost btn-icon btn-sm rounded-circle texttooltip" data-template="trashOne" id="trashOne">
