...
import { attendenceALL } from '../utils/Constants';

const AllAttendence = () => {

    const days = [...Array(31).keys()]; // Adjust the number of days as needed
    ...
    console.log(selectedMonth, "select")
    ...
    const getEmployess = async () => {
        const response = await attendenceALL(selectedMonth);
        console.log(response, "Employe att")
        try {
                    ...
                    <div className="col-12">
                        <nav className='p-2 mt-3' style={customStyle} aria-label="breadcrumb">
                            <ol className="breadcrumb">
                                ...
                                <li className="breadcrumb-item active" aria-current="page">Employees Attendances</li>
                            </ol>
                        </nav>
                    </div>
                    <div className="container-fluid mt-3 border p-3 bg-white">
                        <div className="row">
                            <div className="col d-flex align-items-center">
                                <h6 className="mb-0">{getMonthName(selectedMonth)}, {new Date().getFullYear()}</h6>
                            </div>
                            {days.map((day) => (
                                <div
                                    key={day}
                                    className={`col-1 text-center border p-2`}
                                    style={{ margin: '0.1rem' }}
                                >
                                    <div className="font-weight-bold">{day + 1}</div>
                                </div>
                            ))}
                        </div>
                        <div className="row mt-3">
                            <div className="col">
                                <button
                                    ...
                                    onClick={() => setSelectedMonth((prevMonth) => (prevMonth === 1 ? 12 : prevMonth - 1))}
                                >
                                    Previous Month
                                </button>
                            </div>
                            <div className="col text-right">
                                <button
                                    className="btn btn-primary"
                                    onClick={() => setSelectedMonth((prevMonth) => (prevMonth === 12 ? 1 : prevMonth + 1))}
                                >
                                    Next Month
                                </button>
                            </div>
                                    ...
                                    Employees?.map((employ) => {
                                        return (
                                            <tr>
                                                ...
                                                <td>{employ?.month_working_days}</td>
                                                <td>{employ?.holidays}</td>
                                                <td>{employ?.total_leaves}</td>
                                                <td>{employ?.total_working_days}</td>
                                                <td>{employ?.total_working_hours}</td>
