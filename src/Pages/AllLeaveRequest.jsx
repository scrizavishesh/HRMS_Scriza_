...
import { leasveALL } from '../utils/Constants';
...
const AllLeaveRequest = () => {
    ...
    const GetLeaves = async () => {
        const response = await leasveALL();
        try {
            if (response?.status === 200) {
                toast.success(response?.data?.message);
                setLeaves(response?.data?.date)
            } else {
                                                        ...
                                                        return (
                                                            <tr>
                                                                ...
                                                                <td class="ps-1 text-center">
                                                                    <a href="#!">{employ?.company_id}</a>
                                                                </td>
                                                                <td>{employ?.name}</td>
                                                                <td>{employ?.job_position}</td>
                                                                <td>{employ?.apply_date.slice(0, 10)}</td>
                                                                <td>{employ?.leave_from.slice(0, 10)}</td>
                                                                <td>{employ?.leave_to.slice(0, 10)}</td>
                                                                <td>{employ?.leave_type}</td>
                                                                <td>{employ?.leave_remark.slice(0, 50)}...</td>
                                                                <td>
